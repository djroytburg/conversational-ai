...
def MacroGetName(Macro):
    def __init__(self):
        self.request = "The user was just asked for their name. Judging by their response, by what name should the user be addressed?"
        self.full_ex = json.dumps({"name": "steven"})
        self.empty_ex = json.dumps({"name": "no-face"})
    def run(self, ngrams: Ngrams, vars: Dict[str, Any], args: List[Any]):
        if 'name' in vars:
            return vars['name']
        else:
            examples = f'{self.full_ex} or {self.empty_ex} if unavailable'
            prompt = f'{self.request} Respond in the JSON schema such as {examples}: {ngrams.raw_text().strip()}'
            output = gpt_completion(prompt)
            if not output: return False
            try:
